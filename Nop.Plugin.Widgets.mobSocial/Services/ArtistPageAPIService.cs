...
namespace Nop.Plugin.Widgets.MobSocial.Services
{
    public class ArtistPageAPIService: IArtistPageAPIService
    {
        ...
        public IList<string> SearchArtists(string Term, int Count = 15, int Page = 1)
        {
            ...
            var CACHE_KEY = string.Format("MOBSOCIAL_ARTIST_SEARCH_{0}_COUNT_{1}_PAGE_{2}", Term, Count, Page);
        ...
        {            
            ...
            var CACHE_KEY = string.Format("MOBSOCIAL_SONGS_SEARCH_{0}_{1}_COUNT_{2}_PAGE_{3}", Term, Artist, Count, Page);
            return _cacheManager.Get(CACHE_KEY, 24 * 60, () => //cache for 24 hours
            {
